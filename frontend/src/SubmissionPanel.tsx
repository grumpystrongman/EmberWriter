...
type ManuscriptPreset = 'standard_novel' | 'shunn_classic' | 'custom'
...
export default function SubmissionPanel({ apiBase, slug, disabled }: Props) {
  ...
  async function draft(kind: DraftKind) {
    ...
    setError('')
    try {
      const response = await request<{ text: string }>(`${apiBase}/projects/${slug}/submissions/draft`, {
        method: 'POST',
        body: JSON.stringify({
          kind,
          provider,
    ...
    <details className="authoring-panel submission-panel">
      ...
      <div className="authoring-panel-body submission-body">
        ...
        {selectedDestination && (
          <section className="submission-target-card">
            <div className="submission-grid">
              ...
              <label>Type<select value={selectedDestination.kind} onChange={(event) => updateDestination({ kind: event.target.value as TargetKind })}><option value="agent">Agent</option><option value="publisher">Publisher</option><option value="editor">Editor</option><option value="contest">Contest</option><option value="other">Other</option></select></label>
              <label>Contact<input value={selectedDestination.contact_name} onChange={(event) => updateDestination({ contact_name: event.target.value })} /></label>
              <label>Method<select value={selectedDestination.method} onChange={(event) => updateDestination({ method: event.target.value as SubmissionMethod })}><option value="email">Email</option><option value="query_manager">QueryManager</option><option value="web_form">Web form</option><option value="postal">Postal</option><option value="other">Other</option></select></label>
              <label>Email<input value={selectedDestination.email} onChange={(event) => updateDestination({ email: event.target.value })} /></label>
              <label>Submission portal<input value={selectedDestination.submission_url} onChange={(event) => updateDestination({ submission_url: event.target.value })} /></label>
              <label className="submission-wide">Guidelines URL<input value={selectedDestination.guidelines_url} onChange={(event) => updateDestination({ guidelines_url: event.target.value })} placeholder="Current authoritative submission page" /></label>
              <label>Sample<select value={selectedDestination.sample_kind} onChange={(event) => updateDestination({ sample_kind: event.target.value as SampleKind })}><option value="none">None</option><option value="pages">First pages</option><option value="chapters">First chapters</option><option value="words">First words</option><option value="full">Full manuscript</option></select></label>
              {selectedDestination.sample_kind !== 'none' && selectedDestination.sample_kind !== 'full' && <label>Amount<input type="number" min="1" value={selectedDestination.sample_count} onChange={(event) => updateDestination({ sample_count: Number(event.target.value) })} /></label>}
              <label>Delivery<select value={selectedDestination.attachment_mode} onChange={(event) => updateDestination({ attachment_mode: event.target.value as AttachmentMode })}><option value="body">Paste in body/form</option><option value="attachments">Attachments</option><option value="mixed">Mixed</option></select></label>
              <label>Accepted attachment formats<input value={selectedDestination.accepted_formats.join(', ')} onChange={(event) => updateDestination({ accepted_formats: event.target.value.split(',').map((item) => item.trim().replace(/^\./, '')).filter(Boolean) })} /></label>
              <label>Expected response days<input type="number" min="1" max="730" value={selectedDestination.expected_response_days ?? ''} onChange={(event) => updateDestination({ expected_response_days: event.target.value ? Number(event.target.value) : null })} /></label>
              <label>Simultaneous submissions<select value={selectedDestination.simultaneous_submissions_allowed === null ? 'unknown' : String(selectedDestination.simultaneous_submissions_allowed)} onChange={(event) => updateDestination({ simultaneous_submissions_allowed: event.target.value === 'unknown' ? null : event.target.value === 'true' })}><option value="unknown">Unknown</option><option value="true">Allowed</option><option value="false">Not allowed</option></select></label>
            </div>
            <div className="submission-requirements">
              <label><input type="checkbox" checked={selectedDestination.query_required} onChange={(event) => updateDestination({ query_required: event.target.checked })} /> Query required</label>
              <label><input type="checkbox" checked={selectedDestination.synopsis_required} onChange={(event) => updateDestination({ synopsis_required: event.target.checked })} /> Synopsis required</label>
              <label><input type="checkbox" checked={selectedDestination.bio_required} onChange={(event) => updateDestination({ bio_required: event.target.checked })} /> Bio required</label>
            </div>
            <label className="submission-notes">Guideline notes<textarea rows={5} value={selectedDestination.notes} onChange={(event) => updateDestination({ notes: event.target.value })} placeholder="Exact subject line, pasted pages, no attachments, one agent at a time, response window…" /></label>
          </section>
        )}

        {validation && (
          <div className={`submission-validation ${validation.valid ? 'valid' : 'invalid'}`}>
            <strong>{validation.valid ? 'Package passes Ember blocking checks' : 'Submission has blocking issues'}</strong>
        ...
        <div className="submission-actions">
          ...
          <button type="button" className="primary" onClick={() => void build()} disabled={disabled || busy || Boolean(drafting) || !selectedDestinationId || validation?.valid === false}>Build exact submission package</button>
        </div>
          ...
          <details className="submission-history" open>
            ...
            {destinationRecords.slice().reverse().map((record) => (
              <article key={record.id}>
                ...
                <label>Sent<input type="date" value={record.submitted_at.slice(0, 10)} onChange={(event) => void updateRecord(record, { submitted_at: event.target.value })} /></label>
                <label>Follow up<input type="date" value={record.follow_up_on.slice(0, 10)} onChange={(event) => void updateRecord(record, { follow_up_on: event.target.value })} /></label>
                <label>Response<input type="date" value={record.response_at.slice(0, 10)} onChange={(event) => void updateRecord(record, { response_at: event.target.value })} /></label>
              </article>
            ))}
          </details>
        )}
