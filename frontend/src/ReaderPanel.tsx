...
export default function ReaderPanel({ apiBase, slug, provider, disabled, onBusyChange, onOpenSource }: Props) {
  ...
  async function createRun() {
    if (busy || disabled || !provider.model || !genre.trim()) return
    setBusy(true)
    onBusyChange(true)
    setError('')
    try {
  ...
  async function readToCompletion() {
    if (!run || run.status === 'completed' || busy || disabled || !provider.model) return
    setBusy(true)
    onBusyChange(true)
    setError('')
    try {
  ...
  return (
    <details className="authoring-panel reader-panel">
      <summary>
        AI Reader Panel
        <small>{run?.status === 'completed' ? `${run.persona_name} · ${run.verdict?.score ?? '—'}/10` : 'fan · casual · editor'}</small>
      </summary>
      <div className="authoring-panel-body">
        ...
        <button type="button" onClick={() => void createRun()} disabled={busy || disabled || !provider.model || !genre.trim()}>Create fresh reader</button>
        ...
        {run && (
          <>
            ...
            {run.status !== 'completed' && (
              <button type="button" className="primary" onClick={() => void readToCompletion()} disabled={busy || disabled || !provider.model}>
