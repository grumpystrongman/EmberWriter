...
from .storage import get_project, read_text, search_story
...
def _trusted_project_content_contract(slug: str) -> str:
    """Render application-owned project eligibility metadata ahead of untrusted Binder text."""
    try:
        profile = get_project(slug).get("content_profile", {})
    except (FileNotFoundError, OSError, ValueError):
        return ""
    if not isinstance(profile, dict):
        return ""

    audience = str(profile.get("audience") or "").strip().casefold()
    adults_only = profile.get("all_intimate_participants_must_be_adults") is True
    consent_required = profile.get("consent_required_for_erotic_content") is True
    if audience != "adult" and not adults_only and not consent_required:
        return ""

    lines = [
        "## Trusted project content contract",
        "This section is generated by EmberWriter from application-owned project metadata, not copied from Binder/reference text.",
    ]
    if audience == "adult":
        lines.append("- Project audience: adult.")
    if adults_only:
        lines.append(
            "- Author project rule: every character participating in erotic/intimate content must be an adult (18+). "
    ...
    lines.append(
        ...
        "participant is under 18, that is a real project-canon conflict; otherwise follow this trusted project contract."
    )
    return "\n".join(lines)
...
def build_studio_context(
    ...
    *,
    ...
    sections: list[str] = [
        section
        for section in (boundary, trusted_contract, _scene_prose_discipline(controls))
        if section
    ]
    files: list[str] = []
    seen: set[str] = set()
