...
def generation_timeout_seconds() -> float:
    ...
    Phase-specific watchdogs are responsible for normal stall detection. This value exists only to
    prevent a programming error from leaving an orphaned request alive indefinitely. Authors may
    still override it explicitly with EMBER_GENERATION_TIMEOUT_SECONDS for testing or operations.
    """
...
async def guard_generation_request(
    ...
    """Expose ordinary JSON Writer generation to cancellation and an emergency ceiling."""
