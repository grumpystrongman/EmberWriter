...
import { asList, clamp, inferScale, SCALE_OPTIONS, titleCase, withTag } from './living-atlas-helpers'
...
export default function LivingAtlasDetails({
  ...
  if (selectedLocation) {
    ...
    return <section className="living-control-card living-inspector">
      ...
      <div className="living-two">
        ...
        <label>Scale<select value={inferScale(selectedLocation)} onChange={(event) => onUpdateLocation(selectedLocation.id, { tags: withTag(selectedLocation, 'scale:', event.target.value) })}>{SCALE_OPTIONS.map((scale) => <option key={scale} value={scale}>{titleCase(scale)}</option>)}</select></label>
      </div>
      ...
      })}><option value="">Top level</option>{locations.filter((item) => item.id !== selectedLocation.id).map((item) => <option key={item.id} value={item.id}>{item.name}</option>)}</select></label>
      ...
      <div className="living-two">
        <label>Canon<select value={selectedLocation.canon_status} onChange={(event) => onUpdateLocation(selectedLocation.id, { canon_status: event.target.value as AtlasLocation['canon_status'] })}><option value="canon">Canon</option><option value="inferred">Inferred</option><option value="suggested">Suggested</option></select></label>
        <label>Position<select value={selectedLocation.position_status} onChange={(event) => onUpdateLocation(selectedLocation.id, { position_status: event.target.value as AtlasLocation['position_status'] })}><option value="canon">Canon</option><option value="inferred">Inferred</option><option value="suggested">Suggested</option></select></label>
      </div>
  ...
  if (selectedConnection) {
    return <section className="living-control-card living-inspector">
      ...
      <div className="living-two">
        <label>Distance<input type="number" min=".1" step=".1" value={selectedConnection.distance} onChange={(event) => onUpdateConnection(selectedConnection.id, { distance: Math.max(.1, Number(event.target.value) || .1) })} /></label>
